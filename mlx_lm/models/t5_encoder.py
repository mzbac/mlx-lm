...
class MultiHeadAttention(nn.Module):
    ...
    def __call__(
        ...
        return self.out_proj(values_hat), (keys, values) if cache is not None else None
...
class Model(nn.Module):
    ...
    def __call__(self, inputs: mx.array, mask: Optional[mx.array] = None, cache: Optional[List] = None):
        x = self.wte(inputs)
        return self.encoder(x)

    def sanitize(self, weights):
        """Convert weights from HuggingFace format to MLX format"""
    ...
    def unsanitize(self, weights):
        """Convert weights from MLX format back to HuggingFace format for saving"""
        _SHARED_REPLACEMENT_PATTERNS = [
            (".layers.", ".block."),
            (".key_proj.", ".k."),
            (".out_proj.", ".o."),
            ...
            (".ln1.", ".layer.0.layer_norm."),
            (".ln2.", ".layer.1.layer_norm."),
            (".ln3.", ".layer.2.layer_norm."),
            (".ln.", ".final_layer_norm."),
            (
                "encoder.relative_attention_bias.embeddings.",
                "layers.0.layer.0.SelfAttention.relative_attention_bias.",
        ...
        _ENCODER_REPLACEMENT_PATTERNS = [
            (".attention.", ".layer.0.SelfAttention."),
            (".dense.", ".layer.1.DenseReluDense."),
        ]

        new_weights = {}
        for k, w in weights.items():
            # First apply encoder-specific patterns if key starts with encoder
            if k.startswith("encoder."):
                for old, new in _ENCODER_REPLACEMENT_PATTERNS:
                    k = k.replace(old, new)
            
            # Then apply shared patterns
            for old, new in _SHARED_REPLACEMENT_PATTERNS:
                k = k.replace(old, new)
